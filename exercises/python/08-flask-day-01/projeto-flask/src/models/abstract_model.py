...
class AbstractModel:
    ...
    def update(self, data: dict):
        ...
        result = self._collection.find_one_and_update(
            {"_id": self.data["_id"]},
            # substitue os dados retornados pelos que acabaram de ser passados
            {"$set": data},
            return_document=ReturnDocument.AFTER,
            # ReturnDocument.BEFORE retorna os dados antes da atualização
        )
...
#     #db importado de models
#     _collection = db["users"]
    # usando dentro do banco uma coleção chamada users
