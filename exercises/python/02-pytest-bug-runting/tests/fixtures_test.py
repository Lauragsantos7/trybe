...
# A função print escreve no stdout. A fixture capsys tem um método chamado readouterr, que lê as saídas padrão e de erro e retorna um objeto contendo err e out. Em seguida, é só verificar se a saída corresponde ao esperado.

# também pode ser usada para capturar a saída de erro padrão stderr, como no exemplo abaixo:
...
# pode ser para testar uma função que cria arquivos, por exemplo uma função que gera um arquivo de saída. O teste pode criar um diretório temporário com a fixture e chamar a função a ser testada, passando o diretório temporário como argumento. Então, o teste pode verificar se a função criou os arquivos corretamente dentro do diretório temporário.
